<template>
    <v-fade-transition mode="out-in">
        <v-row class="my-15">
            ...
            <v-col md="6">
                <v-btn :color="settings.color" class="white-text float-right" rounded outlined to="foto">
                    Selengkapnya
                </v-btn>
            </v-col>
                ...
                <v-hover v-slot="{ hover }" open-delay="200">
                    <router-link :to="'/foto/album/'+image.slug"  class="text-decoration-none">
                      <v-card :elevation="hover ? 18 : 1" :class="{ 'on-hover': hover }">
                        <v-img :height="settings.defaultImageContentHeight"  :src="require(`../assets/${image.source}`)" :alt="image.alt"
                            class="grey darken-4"></v-img>
                            <v-row>
                                <v-col md="6">
                                    <v-chip class="mx-5 my-5 white--text" small :outlined="settings.outlinedGeneralButton" :color="settings.color"
